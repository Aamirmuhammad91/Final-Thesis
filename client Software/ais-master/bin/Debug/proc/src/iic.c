...
typedef struct
{
	...
	float reserved1;
	float reserved2;
	float p_data[ADC_BUFFER_SIZE / 2];
}header_t;
...
float class;
float				distance;
float				time_of_flight_us;
header_t	header;
adc_t				adc;
fft_t				fft;
uint32_t			cnt_log = 0;			// max: 6.000.000 ms -> 6000 sek -> 100 min -> 1h 40 min
uint8_t				sensor_number	=     0;
float				F1_threshold	=   100;
float				F10_threshold	= 10000;
float				F4_threshold = 10000;
...
void set_min_max_fft_frequency( adc_t *a, fft_t *f, uint16_t freq_min_in, uint16_t freq_max_in, uint32_t w_width_in )
{
...
int start_adc( adc_t *a, int16_t *data, struct iic_s *i )
{
	...
	rp_AcqGetLatestDataRaw( RP_CH_1, &a->buffer_size, data ); // channel A
...
// soft object or hard object
// class 2 --- Soft Object (Human)
// class 1 --- Hard Object (No - Human)
uint8_t box_classificator( feat_t *feat )
{
	uint8_t ret = 0;
	
	ret = ( (feat->F1 > F1_threshold) && (feat->F4 > F4_threshold) )
		? ( (uint8_t)2):( (uint8_t)1);
...
void MeasureThread( void *threadid )
{
	...
	set_min_max_fft_frequency( (adc_t *)&adc, (fft_t *)&fft, FFT_MIN_FREQ, FFT_MAX_FREQ, (uint32_t)FFT_WINDOW_STD );
	...
	header.reserved1		= (float)SW_VERSION;
	...
	while( 1 )
	{
		...
		p_data = (uint16_t *)calloc( ADC_BUFFER_SIZE, sizeof(uint16_t));
		...
		start_adc( (adc_t *)&adc, (int16_t *)p_data, &iic );
		...
		if( retval && ( run != RUN_ADC ) )
		{
			...
			// start the FFT
			calc_fft( (adc_t *)&adc, (fft_t *)&fft, (int16_t *)p_data);
			...
			calc_feature_F5( (fft_t *)&fft, (uint16_t *)p_data, &feat.F5 );
			...
			calc_feature_F8( (fft_t *)&fft, (uint16_t *)p_data, &feat );
			...
			if (object_class == 2)			// for human
			{
				if (++LedCounter2 >= 25)
				{
					...
					LedCounter2 = 0;
					LedCounter1 = 0;
					StartLedThread(blue_light, white_light);
				}		
			}
			else if (object_class == 1) // for no-human
			{
				if (++LedCounter1 >= 25)
				{
					white_light = false;
		...
		switch(run)
		{
			...
			case RUN_DEMO:
				if( (counter_demo++ % 2) == 0 )
				{
					p_data[0] = (uint16_t)(sensor_number);
					p_data[1] = (uint16_t)(object_class);
					p_data[2] = (uint16_t)(distance * 100);
					send_data = true;
				}
				break;
			case RUN_LEARN_NN:
				//feature_vector[0]= feat.F1;
				//feature_vector[1]= feat.F10;
				init_neuronal_network();
				//ret_nn = learn_neuronal_network( &feature_vector[0], &class );
				p_data[0] = (int)ret_nn;
				header.DataLength = 2;
				send_data = true;
				if( ret_nn == 0){
			...
			case RUN_CALC_NN:
				//feature_vector[0]= feat.F1;
				//feature_vector[1]= feat.F10;
				//class = calc_neuronal_network( &feature_vector[0] );
				header.DataLength = 0;
				header.reserved1  = class;
				send_data = true;
				break;
			default:
				run = RUN_NONE;
		...
		if( send_data ){
			...
			memcpy( (uint16_t *)header.p_data, p_data, ADC_BUFFER_SIZE * sizeof(uint16_t));
		...
		if( cnt_log > 0 ){
			cnt_log--;
			sprintf( msg_tx, "%0.2f\t%i\t%.2f\t%.0f\t%.0f\t%.0f\t%.2f\t%.2f\t%.2f\t%.2f\t%0.2f\t%0.2f\n", filter_distance.filt_val, object_class, feat.F1, feat.F2, feat.F3, feat.F4, feat.F5, feat.F6, feat.F7, feat.F8, feat.F9, feat.F10 );
			save_data(msg_tx);
			if( cnt_log == 0 ){
				LED3_OFF;
			}
...
//void *set_led_status(void* vargp)
//{
//	
//
//	pthread_exit(NULL);
...
// public functions
//********************************************
void iic_set_log( int32_t cnt_log_in )
{
	if( cnt_log_in < 0 ){
		init_data();
	}
	else if( cnt_log_in <= 6000 ){
		cnt_log = cnt_log_in * 10;
		( cnt_log > 0 )?(LED3_ON):(LED3_OFF);
	}
